...
"""# Task
Create a complete, working version of the Snake game with a summary of its functionalities.

## Przygotowanie środowiska i inicjalizacja

### Subtask:
Set up the game environment using the `turtle` library, including the game window, initial snake, food, and score.

**Reasoning**:
The subtask requires setting up the game environment using the `turtle` library, including the game window, initial snake, food, and score. This involves importing the library, configuring the screen, creating turtle objects for the snake's head and food, and setting up the score display, all of which can be done in a single Python code block.
"""
...
def narysuj_swiateczna_scene():
    """
    Rysuje pełną świąteczną scenę z Mikołajem, choinką i śnieżnym tłem
    przy użyciu Matplotlib i zwraca obiekt figury.
    """

    # 1. Ustawienia sceny i tła
    # Zwracamy fig i ax, aby Streamlit mógł je poprawnie wyświetlić
    fig, ax = plt.subplots(figsize=(10, 8))

    # Paleta kolorów sceny
    niebo_kolor = '#0B1026'  # Bardzo ciemny granat
    snieg_kolor = '#F0F8FF'  # AliceBlue (złamana biel)
    c_red = '#D62828'      # Czerwony Mikołaj
    c_skin = '#FFDFC4'     # Skóra
    ...
    # Warstwy choinki (zielone trójkąty)
    ...
    # --- ŚWIĘTY MIKOŁAJ (Po lewej stronie) ---
    sx = 6
    sy = 3
    ...
    ax.set_title("Wesołych Świąt!", color='white', fontsize=18, fontweight='bold')

    # Zwracamy całą figurę
    return fig

# --- Główna funkcja Streamlit ---
def main():
    st.set_page_config(page_title="Świąteczna Scena Matplotlib", layout="centered")
    st.title("🎄 Interaktywna Scena Świąteczna (Matplotlib + Streamlit)")
    st.markdown("Ten obrazek został wygenerowany w całości za pomocą biblioteki Matplotlib i wyświetlony przez Streamlit.")

    # Wywołanie funkcji rysującej i przekazanie figury do Streamlit
    fig = narysuj_swiateczna_scene()
    st.pyplot(fig)

    st.caption("Użyto bibliotek: Matplotlib, NumPy, Random, Streamlit.")
